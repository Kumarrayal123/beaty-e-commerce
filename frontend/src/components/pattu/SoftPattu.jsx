...
const SoftPattu = () => {
  const { products, addToCart, currency,SoftPattu } = useContext(ShopContext);
  const [sotpattu, setsoftpattu] = useState([]);
...
//   useEffect(() => {
//     setsoftpattu(SoftPattu.slice(0, 10));
//   }, [SoftPattu]);
useEffect(() => {
    if (SoftPattu) {
        setsoftpattu(SoftPattu);
    }
}, [SoftPattu]); // Add SoftPattu as a dependency

...
// import { SoftSilk } from "../../assets/frontend_assets/assets";

// Modal.setAppElement('#root');

// const SoftSilk = () => {
//   const { products, addToCart, currency,SoftSilk } = useContext(ShopContext);
//   const [softsilk, setsoftsilk] = useState([]);
...
const SoftSilk = () => {
  const { products, addToCart, currency, SoftSilk: softSilkData } = useContext(ShopContext);
  const [softsilk, setsoftsilk] = useState([]);
  const [hoveredProduct, setHoveredProduct] = useState(null);
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 gap-y-6 px-10">
              {softsilk?.map((product) => (
                  ...
                  >
                      <div className="relative">
                          <img
                              src={hoveredProduct === product._id && product.image?.[1] ? product.image[1] : product.image?.[0] || "fallback-image.jpg"}
                              alt={product.name}
                              className="w-full h-auto object-cover transition-transform duration-300 ease-in-out cursor-pointer group-hover:scale-105"
                              onClick={() => handleImageClick(product._id)}
                          />
                          ...
                          <div
                              className="absolute top-3 right-5 p-1 rounded-full shadow-lg cursor-pointer sm:top-3 sm:right-5"
                              onClick={() => product.image?.[0] && handleViewImage(product.image[0])}
                          >
                              <FontAwesomeIcon icon={faMagnifyingGlassPlus} className="h-4 w-4 text-white" />
                          </div>
